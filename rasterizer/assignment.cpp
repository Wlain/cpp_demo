...
#include "matrix4.h"
#include "objLoader.h"
#include "pathTracing/ground.h"
#include "pathTracing/ray.h"
#include "pathTracing/renderer.h"
...
void assignment5()
{
    ...
    auto mesh = std::make_unique<pathTracing::Ground>(verts, vertIndex, 2, st);
    mesh->m_materialType = pathTracing::Object::MaterialType::DIFFUSE_AND_GLOSSY;
    scene.add(std::move(mesh));
    scene.add(std::make_unique<pathTracing::Ray>(Vector3(-20, 70, 20), 0.5));
    scene.add(std::make_unique<pathTracing::Ray>(Vector3(30, 50, -12), 0.5));
