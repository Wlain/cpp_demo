...
CBreakPoint::CBreakPoint(uint32 nID,
    ...
    const_string(szFileName, bRef), m_nBreakPointID(nID), m_nLineNum(nLineNum), m_nFileNameStart(0)
{
    assert(szFileName);
...
CDebugBase::CDebugBase(IDebugHandler* pHandler,
    ...
    m_pHandler(pHandler), m_nRemoteListener(INVALID_SOCKET), m_nRemoteConnecter(INVALID_SOCKET), m_bAllExceptionsBreak(false), m_bUncaughtExceptionsBreak(false), m_bPrintFrame(true), m_pBuf(nullptr), m_bLoopOnPause(false), m_bEnterDebug(false), m_eAttachType(eAT_Detach), m_nExceptionID(1), m_bExpectStep(false), m_bQuit(false), m_nCurFrame(0)
{
    if (!nDebugPort)
        return;
...
void CDebugBase::ListenRemote(const char* strDebugHost, uint16 nDebugPort)
{
    ...
    int32 nVal = 1;
    if (SOCKET_ERROR == setsockopt(m_nRemoteListener, SOL_SOCKET, SO_REUSEADDR, (const char*)(&nVal), sizeof(nVal)))
    {
        closesocket(m_nRemoteListener);
    ...
    struct _
    {
        static void Run(CDebugBase* pThis) { pThis->Run(); }
    };
...
void CDebugBase::Run()
{
    while (!m_bQuit)
    {
        ...
        if (ReceiveRemoteData(szBuffer, nResult))
        {
...
bool CDebugBase::ReceiveRemoteData(char (&szBuffer)[2048], int32 nCurSize)
{
    std::string strBuffer;
...
bool CDebugBase::CheckRemoteCmd()
{
    ...
    //    std::cout << m_szStringSend << std::endl;
...
bool CDebugBase::ProcessCommand(CDebugCmd* pCmd)
{
    ...
    else if (!StrCaseCmp(szCommand, "setBreakpoints"))
    {
        ...
        auto it = m_setBreakPoint.lower_bound(bp);
