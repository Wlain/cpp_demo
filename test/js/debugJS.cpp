...
// CDebugJS
//-----------------------------------------------------
CDebugJS::CDebugJS(const v8::Persistent<v8::Context>& context, const char* strDebugHost, uint16 nDebugPort) :
    CDebugBase(nullptr, strDebugHost, nDebugPort)
{
    v8::Local<v8::Context> localContext = v8::Local<v8::Context>::New(v8::Isolate::GetCurrent(), context);
    v8::Context::Scope context_scope(localContext);
    m_isolate = localContext->GetIsolate();
    ...
    v8_inspector::V8ContextInfo Info(m_isolate->GetCurrentContext(), 1, ContextName);
    m_Inspector->contextCreated(Info);
}

CDebugJS::~CDebugJS() = default;

//=====================================================================
// v8_inspector协议
//=====================================================================
bool CDebugJS::ReceiveRemoteData(char (&szBuffer)[2048], int32 nCurSize)
{
    if (nCurSize < 0)
        return false;
    ...
    {
        bool bResult = CDebugBase::ReceiveRemoteData(szBuffer, nCurSize);
        return bResult;
    }
    return false;
}
...
void CDebugJS::sendResponse(int callId, std::unique_ptr<v8_inspector::StringBuffer> message)
{
    ...
    DEBUG_LOG("*********************res begin**********************\n");
    DEBUG_LOG(m_strUtf8Buffer.c_str());
    DEBUG_LOG("\n");
    DEBUG_LOG("**********************res end***********************\n");
}
