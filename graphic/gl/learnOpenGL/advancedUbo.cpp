...
{
    if (m_uboMatrices != 0)
        CHECK_GL(glDeleteBuffers(1, &m_uboMatrices));
}
...
void AdvancedUbo::initUbo()
{
    ...
    CHECK_GL(glUniformBlockBinding(m_redProgram->getProgram(), uniformBlockIndexRed, 0));
    CHECK_GL(glUniformBlockBinding(m_greenProgram->getProgram(), uniformBlockIndexGreen, 0));
    CHECK_GL(glUniformBlockBinding(m_blueProgram->getProgram(), uniformBlockIndexBlue, 0));
    CHECK_GL(glUniformBlockBinding(m_yellowProgram->getProgram(), uniformBlockIndexYellow, 0));
    // Now actually create the buffer
    CHECK_GL(glGenBuffers(1, &m_uboMatrices));
    CHECK_GL(glBindBuffer(GL_UNIFORM_BUFFER, m_uboMatrices));
    CHECK_GL(glBufferData(GL_UNIFORM_BUFFER, 2 * sizeof(glm::mat4), nullptr, GL_STATIC_DRAW));
    CHECK_GL(glBindBuffer(GL_UNIFORM_BUFFER, 0));
    // define the range of the buffer that links to a uniform binding point
    CHECK_GL(glBindBufferRange(GL_UNIFORM_BUFFER, 0, m_uboMatrices, 0, 2 * sizeof(glm::mat4)));
    ...
    CHECK_GL(glBindBuffer(GL_UNIFORM_BUFFER, m_uboMatrices));
    CHECK_GL(glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(glm::mat4), glm::value_ptr(projection)));
    CHECK_GL(glBindBuffer(GL_UNIFORM_BUFFER, 0));
}
...
void AdvancedUbo::update(float elapseTime)
{
    ...
    CHECK_GL(glBindBuffer(GL_UNIFORM_BUFFER, m_uboMatrices));
    /// 第二个参数代表偏移
    CHECK_GL(glBufferSubData(GL_UNIFORM_BUFFER, sizeof(glm::mat4), sizeof(glm::mat4), glm::value_ptr(view)));
    CHECK_GL(glBindBuffer(GL_UNIFORM_BUFFER, 0));
}
